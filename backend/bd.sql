...
-- Nos regresara un arreglo y tendremos las entregas pendientes para la primera pantalla
CREATE OR REPLACE FUNCTION seleccionar_entregas_entregadas()
RETURNS TABLE (
    cliente TEXT,
    tipo_entrega TEXT
)
...
AS $$
    ...
    JOIN clientes c ON e.id_cliente = c.id_cliente
    JOIN cat_tipos_entrega t ON e.id_tipo_entrega = t.id_tipo_entrega
    WHERE e.id_estado_entrega = 2; -- estado : reagendada
$$;

-- Con esto creamos una entrega sin su repartidor aún, para asi guardalo
CREATE OR REPLACE PROCEDURE insert_entrega_completa(
...
BEGIN
    ...
    INSERT INTO clientes (nombre, telefono) VALUES (p_nombre_cliente, p_telefono_cliente) RETURNING id_cliente INTO cliente_id;
...
BEGIN
    ...
    IF NOTagregar_estado_entrega EXISTS (
...
-- Para liberar al repartidor y vehiculo cuando ya no hay entregas
CREATE OR REPLACE PROCEDURE finalizar_entrega_y_liberar_recursos(
    IN p_id_entrega INT
)
LANGUAGE plpgsql
AS $$
DECLARE
    v_id_repartidor INT;
    v_id_vehiculo INT;
    v_estado_entregada INT;
    v_estado_pendiente INT;
    v_estatus_libre INT;
BEGIN
    -- Obtener IDs de estado 'entregada' y 'pendiente'
    SELECT id_estado_entrega INTO v_estado_entregada
    FROM cat_estados_entrega WHERE nombre = 'entregado';

    SELECT id_estado_entrega INTO v_estado_pendiente
    FROM cat_estados_entrega WHERE nombre = 'pendiente';

    -- Marcar entrega como entregada
    UPDATE entregas
    SET id_estado_entrega = v_estado_entregada
    WHERE id_entrega = p_id_entrega;

    -- Obtener repartidor y vehículo asignados
    SELECT id_repartidor_asignado, id_vehiculo_asignado
    INTO v_id_repartidor, v_id_vehiculo
    FROM entregas
    WHERE id_entrega = p_id_entrega;

    -- Verificar si el repartidor tiene otras entregas pendientes
    IF NOT EXISTS (
        SELECT 1
        FROM entregas
        WHERE id_repartidor_asignado = v_id_repartidor
        AND id_estado_entrega = v_estado_pendiente
    ) THEN
        -- Obtener ID de estatus 'libre'
        SELECT id_estatus_vehiculo INTO v_estatus_libre
        FROM cat_estatus_vehiculo WHERE nombre = 'libre';

        -- Liberar vehículo
        UPDATE vehiculos
        SET id_estatus_vehiculo = v_estatus_libre,
            id_repartidor_actual = NULL
        WHERE id_vehiculo = v_id_vehiculo;

        -- Marcar repartidor como libre
        UPDATE repartidores
        SET activo = TRUE
...
CALL guardar_vehiculo(1, 'Italika 150', 35.0, 1); -- tipo moto, ocupada, asignada a repartidor 4
...
-- Para asignar repartidores a una entrega
call estaticas_a_pendientes();


-- Para tener clientes con entrega estatica
CALL crear_entrega_completa('Angel David', '555-111-2222','Av. Juárez', '101', 'B', 'Centro', 
	'Cuauhtémoc', '06010', 'México',2, 'Un sillon', 4500.00);
CALL crear_entrega_completa('Carlos Aguilar', '555-111-2222','Av. Juárez', '101', 'B', 'Centro', 
	'Cuauhtémoc', '06010', 'México',1, 'Una laptop', 4350.00);
CALL crear_entrega_completa('Pedro Maracas', '555-111-2222','Av. Juárez', '101', 'B', 'Centro', 
	'Cuauhtémoc', '06010', 'México',1, 'Entrega de apoyo alimentario', 450.00);

-- Asignando vehiculo
call asignar_vehiculo_a_repartidor(4,1);

-- ahora si asignando entregas
call asignar_entregas_a_repartidor(4,ARRAY[1,2,5]);
*/
